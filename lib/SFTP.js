module.exports = {
    ...
    connection.sftp(function(err, sftp) {
      ...
      aFiles.forEach(function(file) {
        ...
        sftp.fastGet(file.src, file.dest, function(err) {
          if (err) {
            SFTP.createMessage('Error while downloading : ' + relativePath, relativePath, 'error');
            throw err;
          }
          SFTP.createMessage('Succesfully downloaded ' + relativePath, relativePath, 'success');
          sftp.end();
        });
      });
    });
  ...
  downloadCurrentFile: function() {
    ...
    atom.confirm({
      message: "Do you want to overwrite the current local file with the remote copy?",
